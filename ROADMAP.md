...
| 15.5.1–15.5.12 | SEO and LLM visibility: README, meta/OG/JSON-LD, sitemap, llms.txt, code splitting, prerendering, 104 blog articles, visual enrichment, blog filters, FAQ categorisation, header nav, My Results redesign. | 15.5.5 still pending (15.5.2, the /science page, shipped and was enriched in 17.3) |
...
| 19 | ✅ COMPLETE: Aina Albaida trend-hook articles published (2026-06-23). Four lightweight SEO/divulgative articles drafted by the disclosed AI content agent Aina Albaida (originally the open draft PR #97), now published through the real mechanism (the `blog_posts` table). Migration `db/migrations/030_blog_seed_aina.sql` seeds 4 rows: `film-photography-the-fox` (Fox), `paper-planners-the-tortoise` (Tortoise), `run-clubs-the-dolphin` (Dolphin), `silent-book-clubs-the-octopus` (Octopus), each `status=published`, `category=teams`, `complexity=beginner`, EN-only JSONB (other five locales empty, frontend falls back to en), Unsplash cover credit folded into `content.en` (no `cover_credit` column), disclosed Aina byline in `author` and as a sign-off line. Idempotent via `ON CONFLICT (slug) DO NOTHING`. Each article hooks a live social trend to one of the 12 roles and routes to First Quarter Cèrcol. PR #97 closed as superseded (drafts remain on its branch as history). (2026-08-24 note: the articles ARE live — the migration was applied before the Postgres history was frozen, and their rows travelled to D1 in the Cloudflare migration; `apply-migrations.yml` no longer exists and content changes now go through the blog admin endpoints or `wrangler d1 execute`.) Translations deferred. | Gates: 248 frontend tests + build green (migration not exercised by CI). |
...
<!-- Cleared on 2026-08-24 (docs audit): "Cloudflare Pages migration" was done
     in Phase 20 (2026-08-17, static-assets Worker); the "PageSpeed retest"
     window elapsed in June and its GitHub-Pages baselines are incomparable
     on the current edge (the weekly pagespeed-ingest cron measures
     continuously);
     "Phase 15 — Stripe paywall" was cancelled by docs/policies/pricing.md
     (settled 2026-08-23: nothing is paid); "Phase 15.5.2 — /science" shipped
     in Phase 8 and was enriched in 17.3. -->
