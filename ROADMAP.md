...
| 17.6.7 | Backlog (planned): weekly digest email via Resend, públic subdomini `mcp.cercol.team` si l'ús ho justifica, GSC Crawl Stats ingest, CrUX BigQuery export, og:image per ruta amb el title de la pàgina. Editorial quick wins: enriquir `/blog/creativity-and-personality-what-big-five-research-shows` (pos 9), `/blog/self-other-agreement-big-five-where-gaps-are-biggest` (pos 11), `/da/blog/how-personality-test-scores-are-calculated` (pos 15); keyword research ES per Big Five / personalidad equipo. | |
...
| 17.8 | Extract require_admin to api/deps.py (planned). Eliminar la duplicació de la dependency `_require_admin` que fa avui `api/main.py`, `api/blog.py` i `api/seo.py`. | |
