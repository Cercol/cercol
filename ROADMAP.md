...
| 17.6.1b | SEO data foundation, deploy (planned). Pujar service account key al server, env vars a `/home/cercol/.env`, aplicar DDL una vegada amb `--apply`, instal·lar els 3 crons. Esperar 48h perquè el bulk export de GSC tingui dades i validar les primeres execucions amb `bq query`. Procediment complet documentat a `docs/ops/runbook.md` secció "SEO ingest jobs". | |
| 17.6.2 | GSC Crawl Stats ingest + first dashboards (planned). Job nou `api/jobs/gsc_crawl_stats_ingest.py` que cobreix la limitació identificada a 17.6.1a (el frontend cercol.team no té logs origin, només via GSC). Primer dashboard mínim a `/admin/seo/` amb dades reals. | |
| 17.6.3 | CrUX ingest + dashboard de tendencies (planned). | |
| 17.6.4 | MCP server (ADR 0008) i admin UI rewrite (ADR 0009). | |
